...
void LCD_DisplayCharRotated(uint16_t Xpos, uint16_t Ypos, uint8_t Ascii) {
    Ascii -= 32;
    const uint16_t *charBitmap = &LCD_Currentfonts->table[Ascii * LCD_Currentfonts->Height];

    for (uint16_t row = 0; row < LCD_Currentfonts->Height; row++) {
        for (uint16_t col = 0; col < LCD_Currentfonts->Width; col++) {
            uint16_t bit = LCD_Currentfonts->Width <= 12
                ? ((charBitmap[row] >> (LCD_Currentfonts->Width - 1 - col)) & 0x1)
                : ((charBitmap[row] >> col) & 0x1);
            if (bit) {
                // Rotate -90°: (x, y) -> (y, -x)
                LCD_Draw_Pixel(Xpos - row, Ypos + col, CurrentTextColor);
            }
        }
    }
}
